 ...
 *    - Implement callable hook `reset_on_fork` (which sets flag `got_forked` to indicate in next wrapper call a
 *      necessary removal of map values; called by `reset_values_in_forked_process` in event.c)
 *  - Support multiple traced files in `basic` struct (some function-events affect multiple files) -> `sync`,
 *    `floseall`, `copy_write_data` and `MPI_Waitall`
 *      - Note regarding `copy_write_data`: Implemented using 2 calls to this module -> Check enum during call whether
 *        read or write data + assemble it
 *  - Fildes 'extracted' from Streams (via `fileno`) must be deleted as soon as the original stream gets closed (e.g.,
 *    via `fclose`) for more precise tracing (currently not traced function creating same fildes may result in wrong
 *    `traced_filename`)
 *  - DL_IO, POSIX_AIO
 *    - Things to keep in mind regarding DL_IO:
 *      - Additionally tracing DL_IO might incur a SIGNIFICANT increase in open files -> fnmap's  initial size of 100
 ...
 *
 * KNOWN ISSUES:
 *  - `fnmap_destroy` currently LEAKS MEMORY since `__del_hook` isn't executed for each item in fnmap (not a very
 *    serious issue though since the function will only be called once the observed program exits, i.e., the OS will
 *    cleanup)
 *  - On an `exec*` call, the global file-map (of the process) will be overwritten, removing fildes which WILL BE
 *    inherited (since they weren't opened w/ `O_CLOEXEC` flag) by the new (replaced) executable
...
static void create_fnmap_key_using_vals(file_handle_type_t type, void *id_ptr, size_t mmap_length,
                                        fnmap_key_t *new_key_ptr);
static void create_fnmap_key_using_ioevent_file_type(struct basic *ioevent_ptr, fnmap_key_t *new_key_ptr);
static void create_fnmap_key_using_ioevent_function_data(struct basic *ioevent_ptr, fnmap_key_t *new_key1_ptr,
                                                         fnmap_key_t *new_key2_ptr);

static const char *get_file_name_from_ioevent_function_data(struct basic *ioevent_ptr);

/* -- Macros -- */
#define SET_TRACED_FNAME_FOR_IOEVENT(IOEVENT_PTR, TRACED_FNAME)                                            \
    do {                                                                                                   \
        (IOEVENT_PTR)->traced_filename[sizeof((IOEVENT_PTR)->traced_filename) - 1] =                       \
            '\0'; /* ensure there's a terminating '\0' after strncpy */                                    \
        strncpy((IOEVENT_PTR)->traced_filename, (TRACED_FNAME),                                            \
                sizeof((IOEVENT_PTR)->traced_filename) - 1 /* save space for terminating '\0' */);         \
        DEV_DEBUG_PRINT_MSG("Set for ioevent >>%s<< traced_filename \"%s\"", (IOEVENT_PTR)->function_name, \
                            (IOEVENT_PTR)->traced_filename);                                               \
    } while (0)

#define RETURN_IF_IOEVENT_FAILED(IOEVENT_PTR, RTN_STATUS)                   \
    do {                                                                    \
...
#define ADD_OR_UPDATE_FNAME_IN_TRACE_USING_IOEVENT_FILE_TYPE(IOEVENT_PTR, FILENAME) \
    do {                                                                            \
...
#define ADD_OR_UPDATE_FNAME_IN_TRACE_USING_IOEVENT_FUNCTION_DATA(IOEVENT_PTR, FILENAME)                      \
    do {                                                                                                     \
...
#define RMV_FNAME_FROM_TRACE_USING_IOEVENT_FILE_TYPE(IOEVENT_PTR)             \
    do {                                                                      \
...
#define ELSE_SET_FNAME_NOT_FOUND(IOEVENT_PTR, RTN_STATUS)                      \
    else {                                                                     \
...
void fnres_init(long fnmap_max_size) {
    if (fnmap_is_inited()) {
        DEV_DEBUG_PRINT_MSG(
            "Got already init");    // NOTE: Don't throw error here (since it may be called n times -> `fork`)
        return;
    }
...
/**
 * Finalizes, i.e., "un"initializes module; should be called prior termination for cleaning up  (by `cleanup_process` in
 * event.c)
 */
void fnres_fin(void) {
    fnmap_destroy();
...
int fnres_trace_ioevent(struct basic *ioevent_ptr) {
    ...
    switch (hash_string(extracted_fctname)) {
    ...
    case CASE_FREOPEN64: {
        ...
        if (NULL == extracted_fname) { /* Note: filename == NULL means >> reopen SAME file again << */
            ...
            char *search_found_fname;
            IF_FOUND_FNAME_IN_TRACE_USING_IOEVENT_FILE_TYPE(ioevent_ptr, search_key, search_found_fname) {
        ...
        ADD_OR_UPDATE_FNAME_IN_TRACE_USING_IOEVENT_FILE_TYPE(
            ioevent_ptr, extracted_fname); /* Add under same key but w/ different filename */
        return rtn_status;
    }
    ...
    case CASE_MMAP:
    case CASE_MMAP64: {
        if (((struct memory_map_function *)(ioevent_ptr->__function_data))
                ->map_flags.anonymous) { /* Not file backed (`mmap` will ignore its arg `fd`) */
    ...
    case CASE_FCNTL:
        if (__void_p_enum_cmd_data_dup_function ==
            ((struct fcntl_function *)ioevent_ptr->__function_data)->__void_p_enum_cmd_data) {
            goto case_dup;
        }
    ...
    case CASE_FDOPEN: /* Fildes -> Stream */
    {
        fnmap_key_t search_key;
        char *search_found_fname;
        IF_FOUND_FNAME_IN_TRACE_USING_IOEVENT_FUNCTION_DATA(ioevent_ptr, search_key, search_found_fname) {
            SET_TRACED_FNAME_FOR_IOEVENT(ioevent_ptr, search_found_fname);
    ...
    case CASE_MPI_WAITALL: /* ... TODO: Removes only requests from fnmap (to prevent leak), but doesn't set
                              traced_filename(s) ...  */
    {
        const struct mpi_waitall *ioevent_function_data = (struct mpi_waitall *)ioevent_ptr->__function_data;

        if (NULL == ioevent_function_data->__requests) {
            return rtn_status;
        }
    ...
    case CASE_EXECVP:
    case CASE_EXECVPE:
        LOG_WARN("exec detected: Internal mappings pertinent for filename tracing will be overwritten (which may "
                 "affect filename tracing)");
        SET_TRACED_FNAME_FOR_IOEVENT(ioevent_ptr, FNAME_SPECIFIER_NAF);
        return rtn_status;
    ...
    case CASE_COPY_FILE_RANGE: /* Special case: This module will be called twice w/ same `basic` struct, BUT first w/
                                  `copy_read_data` and finally w/ `copy_write_data` for `function_data` */
        goto not_implemented_yet;
    ...
    case CASE_DLOPEN:
    case CASE_DLMOPEN: {
        const char *const extracted_fname = get_file_name_from_ioevent_function_data(
            ioevent_ptr); /* Note: `file_name` may be NULL (causes `dlopen` to return pointer to running program (i.e.,
                             itself)) */
...
static void create_fnmap_key_using_ioevent_file_type(struct basic *ioevent_ptr, fnmap_key_t *new_key_ptr) {
    if (NULL != ioevent_ptr->__function_data &&
        ...
        create_fnmap_key_using_vals(
            F_DESCRIPTOR, &((struct fstatat_function *)ioevent_ptr->__function_data)->file_descriptor, 0, new_key_ptr);
        return;
    }
    switch (ioevent_ptr->__void_p_enum_file_type) {
    case __void_p_enum_file_type_file_descriptor:
        create_fnmap_key_using_vals(F_DESCRIPTOR, &((struct file_descriptor *)ioevent_ptr->__file_type)->descriptor, 0,
                                    new_key_ptr);
        return;

    case __void_p_enum_file_type_file_stream:
        create_fnmap_key_using_vals(F_STREAM, ((struct file_stream *)ioevent_ptr->__file_type)->stream, 0, new_key_ptr);
        return;

    case __void_p_enum_file_type_file_dir:
        create_fnmap_key_using_vals(F_DIR, ((struct file_dir *)ioevent_ptr->__file_type)->directory_stream, 0,
                                    new_key_ptr);
        return;

    case __void_p_enum_file_type_file_memory:
        create_fnmap_key_using_vals(F_MEMORY, ((struct file_memory *)ioevent_ptr->__file_type)->address,
                                    ((struct file_memory *)ioevent_ptr->__file_type)->length, new_key_ptr);
        return;

    case __void_p_enum_file_type_file_mpi:
        create_fnmap_key_using_vals(F_MPI, &((struct file_mpi *)ioevent_ptr->__file_type)->mpi_file, 0, new_key_ptr);
        return;

    case __void_p_enum_file_type_request_mpi:
        create_fnmap_key_using_vals(R_MPI, &((struct request_mpi *)ioevent_ptr->__file_type)->request_id, 0,
                                    new_key_ptr);
        return;
...
#endif /* WITH_ALLOC */
        LOG_DEBUG("Unhandled case for `ioevent->__void_p_enum_file_type` w/ value %u",
                  ioevent_ptr->__void_p_enum_file_type);
        return;
    default:
        LOG_WARN("Unknown case for `ioevent->__void_p_enum_file_type` w/ value %u",
                 ioevent_ptr->__void_p_enum_file_type);
        return; /* Note: Currently NOT checked by caller (-> proceeding w/o checking return value might lead to
                   nonsensical fnmap-key; reasoning: indicates incomplete / faulty tracing, hence only warning)  */
    }
}
...
static void create_fnmap_key_using_ioevent_function_data(struct basic *ioevent_ptr, fnmap_key_t *new_key1_ptr,
    ...
    case __void_p_enum_function_data_dup_function:
        create_fnmap_key_using_vals(
            F_DESCRIPTOR, &((struct dup_function *)ioevent_ptr->__function_data)->new_descriptor, 0, new_key1_ptr);
        return;

    case __void_p_enum_function_data_dup3_function:
        create_fnmap_key_using_vals(
            F_DESCRIPTOR, &((struct dup3_function *)ioevent_ptr->__function_data)->new_descriptor, 0, new_key1_ptr);
        return;

    case __void_p_enum_function_data_fileno_function:
        create_fnmap_key_using_vals(
            F_DESCRIPTOR, &((struct fileno_function *)ioevent_ptr->__function_data)->file_descriptor, 0, new_key1_ptr);
        return;

    case __void_p_enum_function_data_fdopen_function:
        create_fnmap_key_using_vals(F_DESCRIPTOR, &((struct fdopen_function *)ioevent_ptr->__function_data)->descriptor,
                                    0, new_key1_ptr);
        return;

    case __void_p_enum_function_data_accept_function:
        create_fnmap_key_using_vals(
            F_DESCRIPTOR, &((struct accept_function *)ioevent_ptr->__function_data)->new_descriptor, 0, new_key1_ptr);
        return;
    ...
    case __void_p_enum_function_data_socketpair_function: /* Note: Creates 2 keys (2 fildes) */
        create_fnmap_key_using_vals(
            F_DESCRIPTOR, &((struct socketpair_function *)ioevent_ptr->__function_data)->descriptor1, 0, new_key1_ptr);
        create_fnmap_key_using_vals(
            F_DESCRIPTOR, &((struct socketpair_function *)ioevent_ptr->__function_data)->descriptor2, 0, new_key2_ptr);
        return;
    ...
    case __void_p_enum_function_data_memory_remap_function:
        create_fnmap_key_using_vals(
            F_MEMORY, ((struct memory_remap_function *)ioevent_ptr->__function_data)->new_address,
            ((struct memory_remap_function *)ioevent_ptr->__function_data)->new_length, new_key1_ptr);
        return;
    ...
    case __void_p_enum_function_data_mpi_open_function:
        create_fnmap_key_using_vals(F_MPI, &((struct file_mpi *)ioevent_ptr->__function_data)->mpi_file, 0,
                                    new_key1_ptr);
        return;
    ...
    case __void_p_enum_function_data_copy_read_function:
        create_fnmap_key_using_vals(F_DESCRIPTOR,
                                    &((struct copy_read_function *)ioevent_ptr->__function_data)->to_file_descriptor, 0,
                                    new_key1_ptr);
        return;
...
#endif
        LOG_DEBUG("Unhandled case for `ioevent->__void_p_enum_function_data` w/ value %u",
                  ioevent_ptr->__void_p_enum_function_data);
        return;
    default:
        LOG_WARN("Unknown case for `ioevent->__void_p_enum_function_data` w/ value %u",
                 ioevent_ptr->__void_p_enum_function_data);
        return;
    }
}
...
static const char *get_file_name_from_ioevent_function_data(struct basic *ioevent_ptr) {
    ...
    default:
        LOG_WARN("Unknown case for `ioevent->__void_p_enum_function_data` w/ value %u",
                 ioevent_ptr->__void_p_enum_function_data);
        return NULL; /* Note: Currently NOT checked by all callers (ISSUE: Has sometimes special meaning, e.g., for
                        `dlopen`; Note: Proceeding w/o checking return value might lead to SIGSEGV)  */
    }
}
